    ...
    describe('Instance methods', function () {
      ...
      describe('getDaysInMonth', function () {
        it('lists days of given month', function () {
          expect(sequence(1, 12).map(function (month) {return fromDate(leapYear, month, 1).getDaysInMonth()})).to.eql(leapYearMonths)
          expect(sequence(1, 12).map(function (month) {return fromDate(normalYear, month, 1).getDaysInMonth()})).to.eql(normalYearMonths)
        })
      })
      ...
      describe('getDayInYear', function () {
        it('returns number between 0-365 depending on distance of 1st jan of same year', function () {
          ...
          expect(sequence(2000, 2010).map(function (year) { return fromDate(year, 12, 31).getDayInYear() })).to.eql(yearSizes2000_2010)
        })
      })
